...
$code=$_GET['code'];
$pdostat= $objetpdo->prepare("SELECT *FROM rendvous where id='$code'");
$v=$pdostat->execute();
$par=$pdostat->fetch();
...
$modif=$objetpdo->prepare ("UPDATE rendvous SET dates='$dat',heurd='$heur',heurf='$heurf',
jour='$jr',duree='$dr',patient='$pate',secretair='$secr',medecin='$mede' where (id='$code')" );
$v=$modif->execute();
if ($v==true) {
    	echo "<script>
    		alert('modification reussit');
 		window.location.href='affrv.php'</script>";
     	}
    	else{
    		echo"erreur";
        }
...
?>
<center>
    <p><a href="affrv.php"></a></p>
    <h1 style="color:red;">AJOUTE randez_vous</h1>

<form action="" method="POST">
  <label>Date</label><br>
  <input type="date" name="dat" value= <?=$par['dates']?>>
        <br>
  <label>Heur Debut</label><br>
  <input type="heur" name="heur" value= <?=$par['heurd']?>>
        <br>
        <label>Heur Fin</label><br>
  <input type="heur" name="heurf" value= <?=$par['heurf']?>>
        <br>
        <label>Jour</label><br>
  <input type="heur" name="jour" value= <?=$par['jour']?>>
        <br>
        <label>Duree</label><br>
  <input type="heur" name="dure" value= <?=$par['duree']?>>
        <br>
